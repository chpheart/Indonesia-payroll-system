...
[高责任 Agent 设计原则]
    本项目涉及薪酬、个税、社保、客户交付、权限和导出，默认按高责任场景设计。任何产品、设计、开发、审查、发布决策都必须显式考虑以下原则：
    - 简单优先：能用确定性规则、固定工作流、表单校验和可审计计算解决的，不上自由自治 Agent。Agent 只负责收集上下文、解释、建议、生成草稿和辅助排障，不能替代规则引擎和审批链。
    - 风险分级先行：新增功能必须在 Product-Spec 或 DEV-PLAN 写清影响对象、数据敏感度、财务/合规后果、可逆性、失败模式和风险等级。说不清风险等级的功能，不准进入开发。
    - 最小权限和最小动作面：Agent/工具只拿完成任务必需的数据和权限。读、写、导出、发送、审批、报税/社保提交、覆盖历史结果等动作分开授权，不给一个工具包打天下。
    - 人工确认卡口：凡是会影响工资结果、税社保金额、客户交付材料、权限范围或外部提交的动作，必须先产出预览、差异和理由，再由有权限的人确认。Agent 不得静默执行高影响写操作。
    - 全链路可追溯：每个员工级结果和每次高影响操作都要能追到输入文件/字段、规则版本、公式链、Agent/用户操作人、时间、审批记录和导出版本。解释不出来的结果等于没做完。
    - 护栏分层：输入侧校验数据来源、权限、格式、缺失和异常；推理侧校验规则适用范围和证据充分性；工具侧校验参数、权限、幂等和影响面；输出侧校验金额平衡、权限泄露、敏感信息和交付格式。
    - 不信任外部内容：客户表格、邮件、网页、PDF、历史备注和模型输出都按不可信输入处理。外部内容里的指令不能覆盖系统规则，不能触发工具调用，不能绕过审批。
    - Eval 和红队常态化：高责任功能必须有黄金案例、边界案例、异常/缺失数据案例、权限越权案例、提示注入/间接注入案例、回归测试和人工抽检样本。只跑 happy path 的验证无效。
    - 运行期监控和安全停机：生产能力必须保留审计日志、异常告警、重算/回滚路径、人工接管路径和数据保全策略。遇到证据冲突、规则缺口或权限不明，默认停下并请求人工处理。
    - 审查验收加严：code-review、bug-fixer、release-builder 对高责任功能必须检查风险分级、权限边界、审批卡口、审计追踪、测试证据、数据隐私和回滚方案；缺一项就不算完成。
