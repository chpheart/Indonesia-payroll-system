...
[发布检查清单]
    ...
    高责任门禁：涉及钱、权限、合规、客户交付、正式导出、外部提交或 Agent 建议的产品，发布前必须有风险分级、最小权限/最小上下文、人工确认卡口、预览/差异、审计追溯、fail closed/人工接管、回滚或更正路径、eval/红队/回归证据；缺任一项不发布。
    隐私审计，绝对底线，对构建产物目录执行：
...
[发布策略]
    Web：构建 → 高责任门禁检查 → 隐私审计 → 配生产环境变量和最小权限 → 部署 Vercel 或 Netlify → 访问 URL 验证无白屏 → 对照 Spec 冒烟测试 → 验证高影响动作在预览、审批、审计、fail closed 和回滚/更正路径上没有绕过。
