...
[审查维度清单]
    ...
    - 高责任门禁：涉及钱、权限、合规、客户交付、正式导出、外部提交或 Agent 建议的功能，必须查风险分级、最小权限/最小上下文、预览差异、人工确认、审计追踪、fail closed/人工接管、回滚或更正路径是否落到代码和测试；缺任一关键门禁按 HIGH 处理
    ...
    - 安全扫描：grep 硬编码密钥、eval、dangerouslySetInnerHTML、字符串拼 SQL、绝对路径、暴露的前缀变量；高责任 Agent 还必须查提示注入/间接注入、不可信外部内容触发工具、敏感字段进 trace/log/model context、审批前生效写操作
